\section{一种新的单细胞基因表达调控网络方法~scGRNHunter}
...
\begin{algorithm}
    ...
    \label{alg:wssmfa}
    \begin{algorithmic}[1]
        \State Input: $X_{G \times C}$                                   \Comment{$G$ genes, $C$ cells, I index set of $n$ regulators}
        \State Output: $L_{G \times K}$, $F_{C \times K}$                \Comment{Loading matrix and factor matrix}
        \While{not converged}
            \For{$i = 1 \to G$}                                              \Comment{For each bootstrapping run}
                \State $l_{i} \leftarrow \min _{l_{i}}\left\|\left(x_{i}-l_{i} F^{T}\right) \odot w_{i}\right\|_{F}^{2}+\alpha\left\|l_{i}\right\|_{1}$ \Comment{Resampling with replacement}
