...
\begin{figure}[!htbp]
    ...
    (E)~用~RafClust~提炼稀有细胞类型。值得注意的是,如果我们不关心稀有细胞的类型,这个步骤就不需要。
...
在本研究中,~RafClust~是作为~DoRC~的一个步骤来解决稀有细胞的异质性,
...
对于实验中采用的~\textit{Jurkat_\293T}~数据集,计算出~Jurkat~细胞的~F1~分数。
...
它们都属于基于近似的方法进行离群点检测。
基于近似的方法假设一个离群物体与其最近邻居的接近程度与该物体与数据集中大多数其它物体的接近程度有很大的差异。
