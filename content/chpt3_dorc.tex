\section{一种新的单细胞稀有细胞及类型识别方法~DoRC}
...
并且对细胞类型特征也很敏感。对该方法的检测实验还表明,DoRC~是可扩展的、快速的。
...
DoRC~方法是用于从超大型~scRNA-seq~数据中发现稀有细胞,包括几个子程序,如图~\ref{fig:flowchart}~所示。
所有子程序的细节在下文中详述。
\begin{figure}[!htbp]
    \centering
...
使用的是调整后的~Rand指数~\cite{hubert1985comparing,wu2005dynamic}:
\begin{equation}
\label{eq:ARI}
...
DoRC的设计灵感来自于稀有细胞往往是``少而不同”的观察, 其中容易受到一种叫做孤立的机制影响。
孤立森林可以充分捕捉稀有细胞的特征,
