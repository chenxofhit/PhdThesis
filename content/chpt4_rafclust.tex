...
因此导致每个细胞很多的基因表达量为 0,
这种现象被称之为单细胞的 dropout \upcite{vallejos2017normalizing}。
同时,单细胞 RNA-seq 测序中将基因作为特征,常见的人和老鼠这两类物种都有 20000 多个基因,
从计算的角度上来说, 高维特征也给单细胞 RNA-seq 数据上的样本聚类带来了挑战 \upcite{stegle2015computational}。
...
抗原特异性 T 细胞对免疫学记忆的形成至关重要 \upcite{slansky2003antigen,altman1996phenotypic,manzo2015antigen}。
来源于骨髓的内皮祖细胞,已被证明是肿瘤血管生成的可靠生物标志物 \upcite{kuo2012dynamics,cima2016tumor}。
干细胞可以替代受损细胞,并用于治疗帕金森氏症、糖尿病、心脏病等疾病 \upcite{jang2005stem}。
循环肿瘤细胞提供了前所未有的视角,为临床管理提供了实时的线索和根据 \upcite{krebs2010circulating}。
...
默认情况下, $\beta$ 设置为 0.06 \upcite{kiselev2017sc3}。 
...
类似于其它的许多方法 \upcite{kiselev2017sc3,pouyan2018random,mohammadi2018geometric,sinha2018dropclust,Srinivasan511626,Li530378,zheng2019sinnlrr},
RafClust 也是两个步骤的方法,
...
每个矩阵中信息量最大的成分由``elbow 法" \upcite{thorndike1953belongs} 保留,
...
RF (随机森林, Random Forest) 是一种应用非常广泛的基于决策树的分类和回归方法 \upcite{breiman2001random}。
它也可以用无监督的方式来推断对象之间的相似性 \upcite{shi2006unsupervised,breiman2011manual,pouyan2018random}。
...
%本文直接使用 R 包 fpc \upcite{package-fpc}中的 \textit{pamk} 函数来获取该特征 $j$ 的向量中的类标签。
...
%本文使用 R 包 randomforest \upcite{package-randomforest},其默认森林大小为 500 棵,即 $N=500$。
...
使用了来自 R 包 dynamicTreeCut \upcite{langfelder2007defining,package-dynamicTreeCut} 的 \textit{cutreeDynamic} 函数自动确定细胞的类别,
并为每个细胞分配正确的类标签。
...
本文采用 NODES \upcite{Sengupta049734} 这一快速的非参数化、差异化表达 (DE) 分析工具进行差异基因分析。
NODES 被证明比传统的基于批量细胞测序的差异分析方法 DESeq2 \upcite{love2014moderated}、edgeR \upcite{robinson2010edger},
以及针对单细胞的差异表达分析方法 scde \upcite{kharchenko2014bayesian} 和 Wilcoxon 秩和检验 (Wilcoxon rank sum test) 都有效 \upcite{Sengupta049734}。
...
一般来说,多级聚类变得至关重要,因为次要的类经常在首次筛选就被过滤掉了 \upcite{campbell2017molecular}。
...
基于它们的相对分散度 (dispersion,即方差/均值) 与具有相似平均表达量的基因之间的预期分散度 \upcite{zheng2017massively,macosko2015highly}选出 1000个变化最大的基因。
...
孤立森林是一种无模型算法,它的计算效率很高,非常适合并行计算方法的使用 \upcite{hariri2018batch}。
事实证明,它在检测异常方面也是非常有效的 \upcite{susto2017anomaly}。
该算法的主要优越性在于,它并不依赖于为数据设置复杂的参数配置。
相反,它利用了异常数据``少而不同"的特点。
其它大多数异常检测算法 (anomaly detection algorithms) 都是通过了解异常数据的属性分布,并将其从其它正常数据样本中分离出来,从而找到异常数据 \upcite{noto2010anomaly,chen2011ordinal,das2016incorporating}。
...
孤立森林 \upcite{liu2008isolation,liu2012isolation} 的算法思想同任何基于树结构的聚合 (ensemble) 方法一样,
也是在基于决策树结构之上的。
在训练时,给定一个维度为 $N$ 的数据集,
...
\end{equation}
其中 $H(i)$ 为谐波数,
可由 $ln(i) + 0.5772156649$ (欧拉常数) \upcite{liu2012isolation}估计,$n$ 为建树时所用的数据点数。
$s(x,n)$ 的值接近 1 表示异常,远小于 0.5 表示正常观测值。
本文在这里使用的参数默认值与文献 \upcite{liu2008isolation,liu2012isolation}一样,
...
每个数据集以第一作者的姓氏命名如下: 
Biase \upcite{biase2014cell},
Treutlein \upcite{treutlein2014reconstructing}, 
Pollen \upcite{pollen2014low}, 
Kolod \upcite{kolodziejczyk2015single}, 
Usoskin \upcite{usoskin2015unbiased}, 
Darmanis \upcite{darmanis2015survey}, 
Goolam \upcite{goolam2016heterogeneity}, 
Li \upcite{li2017reference},
Tasic \upcite{tasic2016adult}, 
Zeisel \upcite{zeisel2015cell}。
...
\begin{table}[!htbp]
  ...
  \midrule
  Biase     &GSE57249     &SMARTer    &FPKM   &56	     &25 734     &5         &\upcite{biase2014cell}\\
  Treutlein &GSE52583  	&SMARTer   	&FPKM  	&80      &23 271     &5 		&\upcite{treutlein2014reconstructing}\\
  Pollen    &SRP041736 	&SMARTer   	&TPM   	&301     &23 730     &11 		&\upcite{pollen2014low} \\
  %Patel     &GSE57872     &Smart-Seq 	&TPM   	&430     &5 948      &5			&\upcite{patel2014single}\\
  Kolod     &E-MTAB-2600  &SMARTer    &CPM    &704     &38 616     &3         &\upcite{kolodziejczyk2015single}\\
  Usoskin   &GSE59739  	&STRT-Seq  	&RPM   	&622     &25 334     &11 		&\upcite{usoskin2015unbiased}\\
  Darmanis  &GSE67835  	&SMARTer    &CPM    &466     &22 088     &9         &\upcite{darmanis2015survey}\\
  Goolam    &E-MTAB-3321  &Smart-Seq2	&CPM  	&124     &41 480     &5			&\upcite{goolam2016heterogeneity}\\
  Li     	  &GSE81861  	&SMARTer    &CPM   &561     &55 186      &9         &\upcite{li2017reference}\\
  Tasic     &GSE71585     &SMARTer    &RPKM   &1 679   &24 057     &18        &\upcite{tasic2016adult}\\
  Zeisel    &GSE60361     &STRT-Seq  	&UMI   	&3 005   &19 972     &9			&\upcite{zeisel2015cell}\\
  \bottomrule                   
  \end{tabular}
  }
\end{table}
...
使用的是调整后的 Rand 指数 (Adjusted Rand index, ARI) \upcite{hubert1985comparing,wu2005dynamic}:
\begin{equation}
\label{eq:ARI}
...
将其 ARI 值与其它六种方法进行比较,
包括 RaceID2 \upcite{grun2016novo}, CIDR \upcite{lin2017cidr}, SIMLR \upcite{wang2018simlr}, SAFE \upcite{yang2018safe}, RtsneKmeans \upcite{hartigan1979algorithm,maaten2008visualizing,van2014accelerating}, RAFSIL \upcite{pouyan2018random}。
...
\begin{table}[!htbp]
  ...
  \resizebox{\columnwidth}{!}{%
      ...
      \midrule
      CIDR (v0.1.5)                                                              & PCA dimension reduction based on zero-imputed similarities, followed by hierarchical clustering                                                                    & \upcite{lin2017cidr}         \\
      \begin{tabular}[c]{@{}l@{}}RaceID2 (March 3, \\ 2017 version)\end{tabular} & K-medoids clustering based on Pearson correlation dissimilarities                                                                                                  & \upcite{grun2016novo}        \\
      RtsneKmeans                                                                & \begin{tabular}[c]{@{}l@{}}t-SNE dimension reduction (initial PCA dim=50, t-SNE dim=3, perplexity=30) and K-means \\ clustering with 25 random starts\end{tabular} & \upcite{hartigan1979algorithm, maaten2008visualizing,van2014accelerating}        \\
      SAFE (v2.1.0)                                                              & Ensemble clustering using SC3, CIDR, Seurat and t-SNE + K-means                                                                                                     & \upcite{yang2018safe}         \\
      SIMLR                                                                      & An appropriate cell to cell distance metric by multi-kernel learning, followed by spectral clustering                                                                                                                                                          & \upcite{wang2018simlr}         \\
      RAFSIL                                                                     & Random forest based cell to cell similary learning, followed by K-means or hierarchical clustering                                                                                                                                                              & \upcite{pouyan2018random}        \\
      \bottomrule
  \end{tabular}
  }
\end{table}
...
% 第一个数据集 \textit{PBMCs\_68k}由 68579 个从健康供体收集的 PBMCs 组成 \upcite{zheng2017massively}。
% 11 个纯化的 PBMCs 亚群的单细胞表达谱被用作细胞类型标签的参考。
% 该数据集可在 \url{www.10xgenomics.com} 下载。

%第二个数据集textit{MSE/20k}包含了来自小鼠大脑Arc-ME区域周围的 ${sim} 20$k scRNA-seq图谱 \upcite{campbell2017molecular}。
%作者将神经元细胞分为34个簇。
%和非神经元细胞通过二传聚类方法分为30个聚类。
%本文只关注30个非神经元集群,其中包括8092个非神经元细胞。
%数据集 (GSE93374) 是从\url{https://hemberg-lab.github.io/scRNA.seq.datasets/mouse/brain/}下载的。

% 第二个数据集 \textit{Jurkat\_293T} 是由 Jurkat 和 293T 的两个表达谱构建的,同样来自同一研究 \upcite{zheng2017massively}。
...
% 通过使用 R 包 Splatter \upcite{zappia2017splatter},由 500 个细胞组成。
...
本文将 DoRC 应用于包含 ${\sim}68$k 外周血单核细胞的 scRNA-seq 数据集 (PBMCs) 的标注进行比对。
 \textit{PBMCs\_68k}数据集是知名的包含有 11个 纯化的免疫细胞亚型的数据集,由 68579 个从健康供体收集的 PBMCs 组成 \upcite{zheng2017massively},
...
据报道,巨核细胞只含有整个细胞集的 0.3\% \upcite{zheng2017massively}。
...
\begin{figure*}[!htbp]
    ...
    % (A) t-SNE based 2D embedding of the data with color-marked cluster identities as reported by Zheng et al. \upcite{zheng2017massively}. 
...
\begin{figure*}[!htbp]
    ...
    \caption{
    % DoRC discovers cells with varying degrees of rarity. In the ${\sim}68$k PBMC data \upcite{zheng2017massively}, minor cell populations with different grades of rarity show up with an
    ...
    DoRC 发现了不同稀有度的细胞。在 ${\sim}68$k PBMC 数据 \upcite{zheng2017massively} 中,不同级别的稀有度对应了一个数量不断增加的稀有细胞群。
...
树突状细胞 (DC) 在感知、吞噬和和抗原监视 \upcite{villani2017single} 方面起着至关重要的作用。
DCs 是最罕见的免疫细胞类型之一,在 PBMCs \upcite{zheng2017massively} 数据集上占比约 0.5\%。
Villani \upcite{villani2017single} 的研究划分了六种不同的树突状细胞的亚型。
...
C4、C5、C9 和 C11 完全由树枝状细胞组成,根据 Zheng 等人提供 \upcite{zheng2017massively}的标注,
...
通过将本文的差异基因与 Villani \upcite{villani2017single} 报道的基因进行叠加。
本文可以有信心地解析 6 个亚型中的 5 个 (DC1、DC2、DC4、DC6) (图 \ref{fig:dorc_dendritic} D、表 \ref{supp-tbl:dendritic})。
从 \upcite{villani2017single} 可以看出, DC5 是未识别的 (unresolved) 的,因为该类型是新分离出来的罕见类型。
在 t-SNE 二维嵌入图中, DC3 与 DC2 属于同一类 \upcite{maaten2008visualizing}。
在 DoRC 检测到的稀有细胞上使用 RafClust 进行聚类,不能完全划分出树突状细胞亚型正确的数量。
然而,在最初 \upcite{zheng2017massively}的研究中, DC1 和 DC4 也没有被聚类算法识别到。
事实上,在原始文献 \upcite{zheng2017massively} 实验结果的 t-SNE 图上,
...
\begin{figure}[!htbp]
    ...
    % With the annotation given by \upcite{zheng2017massively}, C4, C5, C9, C11 are mostly dendritic cells sub types.
    % Dots with the same color represent one subpopulation by clustering the rare cells with RafClust.
    % For each subpopulation, the cells annotated with asterisks marking are also dendrotic cells according to the annotation given by \upcite{zheng2017massively}. 
    ...
    % 通过 \upcite{zheng2017massively} 给出的标注, C4、C5、C9、C11 多为树枝状细胞亚型。
    % 用 RafClust 聚类稀有细胞,同种颜色的点代表一个亚群。
    % 对于每个亚群,根据 \upcite{zheng2017massively} 给出的注释,
    % 用星号标记的细胞也是树突状细胞。
...
\begin{table}[!htbp]
    \centering
    \caption{
        %The dendritic cell types correspond to class types reported by Villani \upcite{villani2017single}
        树枝状细胞类型对应于 Villani 报告的细胞类别 \upcite{villani2017single}
        }
...
本文设计了一个在模拟数据集 Jurkat\_293T 上的实验来评估 DoRC 在细胞类型识别上的性能表现。
 \textit{Jurkat\_293T} 是由 Jurkat 和 293T 的两个表达谱构建的,同样来自同一研究 \upcite{zheng2017massively}。
Jurkat 数据集由 3258 个细胞组成,而 293T 数据集由 2885 个细胞组成。
首先,从 293T 数据集中不放回抽样 1500 个细胞。
然后,通过从 Jurkat 数据集中取样不同数量的细胞,
产生 8 个数据集,其中 Jurkat 细胞数占比分别为 0.5\%、1\%、1.5\%、2\%、2.5\%、5\%、10\%、15\%。
...
DoRC 的核心算法孤立森林 (Isolation Forest) 的时间复杂度为 $O(ntlog\psi)$ \upcite{liu2008isolation}。
...
分别统计出 DoRC、FiRE、GiniClust、LOF \upcite{breunig2000lof} 和 RaceID 所消耗的时间,
...
DoRC 由 Python 实现,使用 scikit-learn包 (版本 0.20.2) \upcite{pedregosa2011scikit}和 pyod包 (版本0.6.7)  \upcite{zhao2019pyod}。
...
本文也直接使用 Pyod 包 (0.6.7版本) \upcite{zhao2019pyod} 的 Python 实现 LOF。
...
% 近来,单细胞转录组学大大改善了本文对细胞表型性质的理解,
% 并且,还加速了新细胞类型的发现。
% 这些新的细胞类型大多是罕见的,因为显然一种丰富的细胞类型如果在很长一段时间内不被本文观察到是很不可能的。
% 一个真正稀有的细胞类型只有通过分析成千上万的细胞才能被发现。
% 虽然过去几年的技术进步使本文能够进行超高通量的单细胞测序,
% 可扩展性的稀有细胞检测方法几乎不存在。
% DoRC 避免了使用聚类作为中间步骤,
...
% 本章中,本文提出了一种高效准确的单细胞聚类方法 RafClust。
% 本文使用多种相似性度量方法刻画细胞的特征,
% 使用随机森林回归模型来学习细胞与细胞之间的相似性矩阵,基于相似性矩阵后采用层次聚类来决定细胞的最终类别。
% 实验结果表明, 在十个单细胞数据集上, RafClust 在 ARI 上表现优于其它六种方法。