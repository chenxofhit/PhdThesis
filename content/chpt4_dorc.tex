...
在单细胞数据上,除了聚类之外,还存在一个十分具有挑战性的问题,即是如何从超大规模的~scRNA-seq~数据中识别稀有细胞。
...
最近基于液滴的单细胞转录组测序技术的发展,使得数以万计的单细胞的并行测序成为可能。
...
虽然异常得分用连续值来表示十分有意义,但有时关于细胞稀有度的二进制注释可以极大地简化分析管道~(pipeline)。
...
然而,孤立森林可以很容易地使用并行计算~\cite{hariri2018batch},因此~DoRC~的性能还有提升空间。
...
我们展示了~DoRC~在~${\sim}68$k~人血细胞单细胞表达谱上结合~RafClust~聚类方法来划分人血树突状细胞亚型的表现。
